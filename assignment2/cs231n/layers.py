...
def affine_forward(x, w, b):
    ...
    # Reshape x into rows
    x_reshaped = np.reshape(x, (x.shape[0], -1))  # Flatten the input while keeping the batch size (N)
...
def layernorm_backward(dout, cache):
    ...
    # Backpropagate through scale and shift
    dbeta = np.sum(dout, axis=0)
    ...
    dmean = np.sum(dx_normalized * -1 / np.sqrt(variance + eps), axis=1, keepdims=True) + dvariance * np.sum(
        -2 * (x - mean), axis=1, keepdims=True) / D
    dx = dx_normalized / np.sqrt(variance + eps) + dvariance * 2 * (x - mean) / D + dmean / D
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_pool = int((H - FH) / stride + 1)
    W_pool = int((W - FW) / stride + 1)

    out = np.zeros((N, C, H_pool, W_pool))
    for i in range(H_pool):
        for j in range(W_pool):
            x_mask = x[:, :, i * stride:(i + 1) * stride, j * stride:(j + 1) * stride]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_pool):
        for j in range(W_pool):
            ...
            x_mask_binary = (x_mask == (x_mask_max)[:, :, None, None])
            dx[:, :, i * stride:(i + 1) * stride, j * stride:(j + 1) * stride] = (dout[:, :, i, j])[:, :, None,
                                                                                 None] * x_mask_binary
