    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                ...
                # Gradient update for incorrect classes
                dW[:, j] += X[i]
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    # Compute indicator matrix where (i, j) is 1 if j != y_i and 0 otherwise
    indicator = (margins > 0).astype(int)
    incorrect_counts = np.sum(indicator, axis=1)

    # Gradient contribution from incorrect classes
    indicator[np.arange(num_train), y] = -incorrect_counts
